...
public class Model 
{
    ...
    {       
        ...
        Color[] multiplecolours = new Color[]{
            Color.BLUE,
            Color.GREEN,
            Color.ORANGE,
            Color.BLACK,
        ...
        bricks = new GameObj[row*column];  // make an array big enough for all the bricks
        for (int g=0; g < row; g++) {
            for (int i=0; i < column; i++){bricks[i+(column*g)] = new GameObj(BRICK_WIDTH*(3*i), BRICK_WIDTH*g+M, BRICK_WIDTH*3, BRICK_HEIGHT, multiplecolours[g%multiplecolours.length]);
    ...
    {        
        if ((direction ==-1 && (!(bat.topX <= 0+B))) || direction == 1 && (!(bat.topX>+ width - B - bat.width))){
